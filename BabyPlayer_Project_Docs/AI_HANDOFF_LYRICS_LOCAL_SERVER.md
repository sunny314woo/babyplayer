...
播放、搜索普通歌词、切换歌词、单曲循环和重新进入播放器，都不得自动触发新的 ASR 或 DeepSeek。
重新进入时允许只读恢复 Mac 已有 DeepSeek 缓存，不产生模型调用或费用。
...
- tvOS 当前副本保存在私有 Caches，Mac SQLite 保留 ASR / DeepSeek 服务端结果。Caches
  被清理或分析期间退出播放页导致本地绑定未写完时，tvOS 会按媒体指纹只读拉回 Mac 已有
  DeepSeek 结果并默认采用；普通歌词和 ASR 仍保留为手工切换选项。
- 换歌或返回 BabyPlayer 首页不会取消已发起的 ASR→DeepSeek 任务；回到原曲目时会恢复进度或结果。
- 如果用户强制结束整个 BabyPlayer 进程，tvOS 内客户端串联任务不再运行；Mac 已完成的缓存仍保留。
...
- tvOS 模拟器自动化测试：60 项通过；真机需继续验证 AVKit 方向键焦点手感。
- Python 服务端：55 项通过，0 失败。
...
- 第 12 首 `I Am The Music Man`：已完成 577.99 秒混合对话/歌曲 ASR，DeepSeek
  `asr_only` 结果为 47 行（3.63–566.39 秒），Mac 缓存只读接口已验证命中。
