...
> 状态：Phase A 实体 Apple TV 功能链路已通，正常首页与默认源持久化已接入；完整性能/故障矩阵和许可证评审待完成
> 日期：2026-09-05
...
> 2026-09-05 当前实现说明：实体机现有 Jellyfin 与 Samba 已确认是同一套 78 个文件。为立即兼容已保存数据，先使用带 `filename-sha256:` 类型前缀、且在各目录内必须唯一的本地兼容身份，持久保存 `sourceID → contentID` 别名；检测到重名则退回来源 ID，不自动共享。这个桥接只覆盖当前已确认的一组媒体，不替代上述完整文件 SHA-256 设计；未来接入任意第三方或不同住宅媒体库前必须完成强身份。
