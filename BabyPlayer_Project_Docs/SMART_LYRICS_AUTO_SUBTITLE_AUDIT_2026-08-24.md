...
- 当前回归：Python 54/54，tvOS 26.2 模拟器 55/55。没有安装或操作真实 Apple TV。
...
7. 当前开发真机走 Debug + Mac HTTP 本地任务；Release 走 HTTPS VPS 并由 Apple TV 提取、上传
   整首临时 M4A。两个执行路径差异很大，Debug 验证通过不能代表 Release 路径已经通过。
...
- tvOS：使用 tvOS 26.2 模拟器和禁止签名方式运行，55 项测试通过。
