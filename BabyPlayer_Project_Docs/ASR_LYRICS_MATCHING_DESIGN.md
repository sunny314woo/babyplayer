...
更新时间：2026-09-05
状态：歌词/字幕用户链路已完成 Debug，当前实体 Apple TV 测试可正常使用。本文描述已实现的个人内测版本，不是未来设想。
...
BabyPlayer 为当前媒体项提供三类可以共存的歌词。已有结果、普通歌词和播放显示可随 Samba/Jellyfin 播放使用；对 Samba 文件生成新 ASR/DeepSeek 结果仍受当前可选 Mac 分析服务输入方式约束，不影响视频播放：
